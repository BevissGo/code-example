...
const ModalFinish = ({ contentContainer, contentCommon, campaignId, positionId }) => {
    ...
    <div className='modal-finish'>
      <ScrollAnimation offset={0} animateIn='animate__fadeIn' duration={0.75} animateOnce={true}>
        <div className='modal-finish__header'>
          ...
          <div className='modal-finish__header__title'>{contentContainer.title}!</div>
          <div className='modal-finish__header__subscription'>{contentContainer.subscription}</div>
        </div>
        <div className='modal-finish__footer flex justify-between'>
          ...
          <Button
            ...
            onClick={() => redirectToWithReplace(history, `/disc-test-campaign-result/${campaignId}/${positionId}`)}
          />
        </div>
      </ScrollAnimation>
